...
SELECT D.DepartmentName, COUNT(A.DepartmentID) AS So_luong_nhan_vien
FROM `Account` A
JOIN `Department` D ON a.DepartmentID = D.DepartmentID
...
GROUP BY Q.QuestionID ORDER BY Q.QuestionID ASC;
...
HAVING COUNT(A.QuestionID) = (
	SELECT MAX(countQues) FROM (SELECT count(B.QuestionID) AS countQues FROM answer B
	GROUP BY B.QuestionID) AS countAnsw
);

-- Câu 9: Thống kê số lượng account trong mỗi group
SELECT 
    G.GroupID, G.GroupName, COUNT(GA.AccountID) AS 'SO LUONG ACCOUNT'
FROM
...
-- Câu 14: Lấy GROUP không thuộc account nào
SELECT * FROM `Group` G
